    ...
    <div class="relative overflow-hidden rounded-3xl ocean-gradient text-white p-6 sm:p-10 shadow-xl shadow-sky-900/10">
        <div class="relative z-10 max-w-2xl">
            ...
            <div class="flex flex-wrap gap-3 mt-6">
                <a href="{{ route('user.kondisi') }}" class="px-5 py-2.5 rounded-xl bg-white text-sky-800 font-bold text-xs hover:bg-sky-50 shadow-md transition">
                    Lihat Grafik Detail &rarr;
                </a>
                <a href="{{ route('user.monitoring') }}" class="px-5 py-2.5 rounded-xl bg-sky-500/30 hover:bg-sky-500/40 text-white font-semibold text-xs border border-white/20 backdrop-blur-sm transition">
        ...
        <div class="space-y-3">
            @foreach($notifications as $notif)
                <div class="p-4 sm:p-5 rounded-2xl border flex items-start gap-3.5 transition shadow-xs
                    {{ $notif->type === 'warning' ? 'bg-amber-50/80 border-amber-200 text-amber-900' : '' }}
                    {{ $notif->type === 'danger' ? 'bg-rose-50/80 border-rose-200 text-rose-900' : '' }}
                    {{ $notif->type === 'info' ? 'bg-sky-50/80 border-sky-200 text-sky-900' : '' }}
                    {{ $notif->type === 'success' ? 'bg-emerald-50/80 border-emerald-200 text-emerald-900' : '' }}">
                    ...
                    <div class="w-8 h-8 rounded-xl flex items-center justify-center shrink-0 mt-0.5
                        ...
                        {{ $notif->type === 'info' ? 'bg-sky-100 text-sky-700' : '' }}
                    ...
                    <div class="flex-1">
                        <div class="flex items-center justify-between">
                            <h2 class="text-sm font-bold">{{ $notif->title }}</h2>
                            <span class="text-[10px] text-slate-500 font-medium">{{ $notif->created_at->diffForHumans() }}</span>
                        </div>
                        <p class="text-xs mt-1 leading-relaxed opacity-90">{{ $notif->message }}</p>
                    </div>
                </div>
            @endforeach
        </div>
    ...
    <div>
        <div class="flex items-center justify-between mb-4">
            <div>
                ...
                <p class="text-xs text-slate-500">Ketinggian air laut relatif terhadap Mean Sea Level (MSL) jam {{ $currentHour }}:00</p>
            </div>
            <a href="{{ route('user.kondisi') }}" class="text-xs font-semibold text-sky-600 hover:text-sky-700">Lihat Semua Grafik &rarr;</a>
        </div>
            ...
            @foreach($locationCards as $card)
                <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-sm hover:shadow-md hover:border-sky-300 transition group flex flex-col justify-between">
                    <div>
                        ...
                        <div class="flex items-start justify-between gap-2">
                            ...
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold
                                {{ $card['status_color'] === 'amber' ? 'bg-amber-50 text-amber-700 border border-amber-200' : '' }}
                                {{ $card['status_color'] === 'blue' ? 'bg-sky-50 text-sky-700 border border-sky-200' : '' }}
                                {{ $card['status_color'] === 'emerald' ? 'bg-emerald-50 text-emerald-700 border border-emerald-200' : '' }}">
                                <span class="w-1.5 h-1.5 rounded-full
                                    {{ $card['status_color'] === 'amber' ? 'bg-amber-500' : '' }}
                                    {{ $card['status_color'] === 'blue' ? 'bg-sky-500' : '' }}
                                    {{ $card['status_color'] === 'emerald' ? 'bg-emerald-500' : '' }}">
                                </span>
                                {{ $card['status'] }}
                            </span>
                        </div>
                        ...
                        <div class="mt-4 flex items-baseline gap-2">
                            <span class="text-3xl font-extrabold tracking-tight {{ $card['current_level'] >= 0 ? 'text-sky-600' : 'text-emerald-600' }}">
                                {{ $card['current_level'] > 0 ? '+' : '' }}{{ $card['current_level'] }}
                            </span>
                        ...
                        <div class="mt-2 h-14">
                            <div id="sparkline-{{ $card['id'] }}" class="w-full h-full"></div>
                        </div>
                        ...
                        <div class="mt-3 pt-3 border-t border-slate-100 grid grid-cols-2 gap-2 text-xs">
                            <div class="bg-slate-50 p-2 rounded-xl">
                                ...
                                <span class="font-bold text-sky-700">+{{ $card['hhw'] }} cm</span>
                            </div>
                            <div class="bg-slate-50 p-2 rounded-xl">
                                <span class="text-[10px] text-slate-400 font-semibold block uppercase">Surut Terendah</span>
                                <span class="font-bold text-emerald-700">{{ $card['llw'] }} cm</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
                        <a href="{{ route('user.kondisi', ['location_id' => $card['id']]) }}" class="font-semibold text-sky-600 hover:underline flex items-center gap-1">
                            <span>Grafik Lengkap</span>
                            <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                        </a>
                        <a href="{{ route('user.monitoring') }}" class="text-slate-400 hover:text-slate-600">
                            Peta Lokasi &rarr;
                        </a>
                    </div>
                </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const cardsData = @json($locationCards);
